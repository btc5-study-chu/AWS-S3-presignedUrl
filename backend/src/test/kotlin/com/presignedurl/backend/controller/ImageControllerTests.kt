...
import com.presignedurl.backend.service.ImageServiceTests
...
class ImageControllerTests{
    ...
    @Test
    fun `createPresignedUrls() を呼んだ時、200 OKでかつ正しい引数でサービス層のcreatePresignedUrlメソッドを呼ぶ`() {
        every { service.createPresignedUrls(any()) } returns listOf(
        ...
        verify (exactly = 1) { service.createPresignedUrls(
            listOf(
    ...
    @Test
    fun `createPresignedUrls() を呼んだ時、正しい返り値を返す`() {
        every { service.createPresignedUrls(any()) } returns listOf(
            PresignedUrl("test1","https://test1"),
            PresignedUrl("test2","https://test2"),
            PresignedUrl("test3","https://test3"),
